...
Your body is made up of cells that contain DNA. Those cells regularly wear out and need replacing, which they achieve by
 dividing into daughter cells. In fact, the average human body experiences about 10 quadrillion cell divisions in a lifetime!
 ...
 */
#ifndef EXERCISM_HAMMING_H
#define EXERCISM_HAMMING_H
#include <string>
#include <stdexcept>
namespace hamming
{
    int compute(const std::string &FirstChain,const std::string &SecondChain)
    {
        size_t FirstChainSize = FirstChain.size();
        if(FirstChain.empty() || FirstChainSize != SecondChain.size())
        {
            throw std::domain_error("Chains should be equal or not Empty");
        }
        int distance =0;
        for(size_t i=0;i<FirstChainSize;i++)
        {
